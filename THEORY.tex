...
Then for a given load increment $\Delta\bs$, there exists a scalar $r$ that can minimise $z$, we denote it as $z^\text{trial}$ since it is computed in the trial state.
In mathematical terms, it can be expressed as
\begin{gather}
    \min\limits_{r\in\mathbb{R}}z^\text{trial},
\end{gather}
...
\begin{gather}
    \begin{split}
        f_s^\text{trial} & =\norm{\beeta^\text{trial}}-z^\text{trial}F_n                                                 \\
                         ...
                         & =\norm{\beeta_n+r\Delta\bs}-z^\text{trial}F_n                                                 \\
                         & =0.
    \end{split}
\end{gather}
...
\begin{enumerate}
    \item If $0<r<1$, the given load increment $\Delta\bs$ contains both elastic unloading and plastic loading.
          Since the elastic unloading portion reduces $z$ to a value smaller than the converged $z_n$, the plastic loading portion shall use this smaller $z$, instead of $z_n$, to correctly compute plasticity development.
...
\begin{breakablealgorithm}
    ...
    \begin{algorithmic}
        ...
        \EndWhile
        \If{$\gamma>0$}\Comment{plastic loading}
        \State compute stress $\bsigma_{n+1}=\bsigma^\text{trial}-2G\gamma\bn_{n+1}$\Comment{$G$: shear modulus}
...
\begin{gather}
    ...
    \end{array}\right.
\end{gather}
Three candidates are proposed, all contain a parameter $u$ to control the evolution rate.
\begin{gather}
