...
\begin{breakablealgorithm}
    ...
    \begin{algorithmic}
        ...
        \State compute trial surface $f_s^\text{trial}$ using $\bsigma^\text{trial}$ and $\left(\cdot\right)_{n}$
        \If{$f_s^\text{trial}\leqslant0$}\Comment{elastic unloading}
        \State update $z$ by solving $\norm{\bs^\text{trial}-a^y_n\balpha_n+\left(z-1\right)\sigma^y_n\bd_n}=\sqrt{\dfrac{2}{3}}z\sigma^y_n$\Comment{$0\leqslant{}z\leqslant1$}
        \State return
        \EndIf
        ...
        \State update $a^y$ and $\sigma^y$ according to $q_{n+1}$\Comment{\eqsref{eq:iso_bone}, \eqsref{eq:kin_bone}}
