...
We shall summarise the changes made to the original formulation before presenting a reference implementation.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[>=latex]
        ...
        \draw[dashed,draw,line width=.3mm](A)circle(4cm);
        \draw[draw,line width=.6mm](C)circle(2.5cm);
        \draw[->,red,line width=1mm](0,0)--(A)node[midway,fill=white,sloped]{$a^y\balpha$};
        \draw[->,dotted,red,line width=.4mm](C)--(B);
        \draw[->,red,line width=1mm](A)--(C);
        \draw[->,red,line width=1mm](C)--++(120:2.5)node[midway,fill=white]{$\beeta$};
        \draw[->,red,line width=1mm](0,0)--($(C)+(120:2.5)$)node[midway,fill=white,sloped]{$\bs=\dev{\bsigma}$};
        \draw[|<->|](C)--++(35:2.5)node[midway,fill=white,sloped]{$zF$};
        \draw[<-]($(A)+(60:4)$)--++(60:1.4)--++(1,0)node[right,fill=white]{yield surface $f$};
        \draw[<-]($(C)+(60:2.5)$)--++(60:1.7)--++(1,0)node[right,fill=white]{subloading surface};
        \draw[<-]($(C)!0.5!(B)$)--++(-60:1.5)--++(2,0)node[right,fill=white]{$z\sigma^y\bd$};
        \draw[<-]($(A)!0.5!(C)$)--++(-60:2)--++(2.5,0)node[right,fill=white]{$\left(1-z\right)\sigma^y\bd$};
    \end{tikzpicture}
...
For $n=1$, it also satisfies the strong condition.
For $n>1$, it is numerically unstable thus shall not be considered.
