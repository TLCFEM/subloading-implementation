...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[>=latex]
        ...
        \draw[|<->|](C)--++(45:\SF)node[midway,fill=white,sloped]{$zF$};
        \draw[<-]($(A)+(60:\FF)$)--++(60:1.4)--++(1,0)node[right,fill=white]{yield surface $f$};
...
\begin{gather}\label{eq:iso_bone}
    ...
    \end{gather}
    It must be pointed out that, strictly speaking, \eqsref{eq:kin_bone} shall be expressed as
    \begin{gather}
    ...
    \end{gather}
    With \eqsref{eq:kin_bone}, the extra constant $\sqrt{\dfrac{2}{3}}$ is combined with the constant $b$ to make it a little more compact.
