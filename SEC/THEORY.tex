...
\end{gather}
By comparing with \eqsref{eq:original_core}, one could conclude that \eqsref{eq:polar_decomposition} must be mathematically equivalent to \eqsref{eq:original_core} if $\bd$ is defined in a manner such that
\begin{gather}
    \dot{\bd}=c_e\gamma\left(z_e\bn-\bd\right).
\end{gather}
This is effectively an Armstrong--Frederick type evolution rule.
...
However, one shall note that the specific form $\bbeta=b\left(\bn-\dfrac{\bbeta}{H}\right)\gamma$, which is used by \citet{Anjiki2019,Anjiki2021}, shall not be used as the back stress may go beyond the bound ($\norm{\bbeta}>H$) for softening materials, see the discussion around \figref{fig:examples}.
