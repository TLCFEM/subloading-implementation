...
\begin{gather}
    f=\norm{\beeta}-F,
\end{gather}
in which $\beeta=\beeta\left(\bsigma,\cdots\right)$ is known as the shifted stress that shall be a function of the stress tensor $\bsigma$ and other internal variables, $F=F\left(q\right)$ characterises the size of this surface and may evolve with the development of plasticity driven by some internal variable $q$.
...
The original proposal of the subloading surface model introduces a new internal variable, named as the normal yield ratio, denoted by $z$ in this work, which effectively scales the yield surface $f$ down by a factor $z$, the new surface is called the subloading surface $f_s$ and it is concentric with the yield surface $f$.
\begin{gather}
    f_s=\norm{\beeta}-zF.
\end{gather}
...
\section{Proposed Revisions}\label{sec:revised}
...
The term $y'$ is governed by the difference $a\left(x\right)-y$, while the approaching rate changes from $b$ to $b/a\left(x\right)$.
...
\end{gather}
After careful comparison, one may observe that \eqsref{eq:strict_bound} is in fact the scalar unidirectional version of the evolution rule presented in \eqsref{eq:original_core}.
...
\end{gather}
By comparing with \eqsref{eq:original_core}, one may conclude that \eqsref{eq:polar_decomposition} must be mathematically equivalent to \eqsref{eq:original_core} if $\bd$ is governed by the following evolution rule,
\begin{gather}
...
\subsection{Revised Evolution Rule (Elastic Core)}
...
\end{gather}
then the evolution of $\bc$, which characterises a point on the elastic core surface, can be equivalently described by the evolution of $\bd$, which can take a very simple form as the contribution of $F$ is separated,
...
\subsection{Revised Evolution Rule (Back Stress)}
...
\end{gather}
with $H=H\left(q\right)$ being the bound and $\balpha$ being the normalised back stress (normalised by $H$) which may have the following evolution rule.
...
However, one shall note that the specific form $\dot{\bbeta}=b\left(\dot{\bvarepsilon^p}-\norm{\dot{\bvarepsilon^p}}\dfrac{\bbeta}{H}\right)$, which is used by \citet{Anjiki2019,Anjiki2021}, shall not be used as the back stress may go beyond the intended bound ($\norm{\bbeta}>H$) for softening materials, see the discussion around \figref{fig:examples}.
\subsection{Shifted Stress}
...
\end{enumerate}
Thus, for any given load increment, one can 1) find the corresponding scalar $r$ that minimises $z^\text{trial}$, 2) use the above criteria to determine whether plasticity occurs, if so, also correct the starting value of $z$ if necessary.
We defer the presentation of explicit expressions for the above procedure after introducing the von Mises framework.
\section{A von Mises Model for Metals}
\subsection{von Mises Framework}
In the following, we present discrete expressions within the von Mises framework for 3D models.
The uniaxial version can be obtained accordingly.
...
\begin{table}[htb]
    ...
    \begin{tabular}{rlll}
        ...
        \multicolumn{4}{l}{Items labelled by \faLightbulb{} are novel proposals compared to the existing formulation \citep{Hashiguchi2024}.}                                   \\ \bottomrule
    \end{tabular}
\end{table}
...
The main takeaway is the proposed decomposition, namely \eqsref{eq:decomposition_core} and \eqsref{eq:decomposition_back}.
...
Similar to the conventional return mapping algorithm, the trial state is computed by assuming the given load increment is fully elastic, then one can compute the trial stress $\bsigma^\text{trial}$ via the constitutive law.
\begin{gather}
...
Accordingly, it is possible to directly compute the ratio $r$ by setting the shifted stress to zero with $z^\text{trial}=0$, that is,
\begin{gather}
...
\begin{gather}
    \pdfrac{\norm{\bzeta}}{\gamma}=\bn:\pdfrac{\bzeta}{\gamma},\qquad
    \pdfrac{\norm{\bzeta}}{z}=\bn:\pdfrac{\bzeta}{z},
\end{gather}
then
...
\begin{gather}
    ...
    \end{bmatrix}.
\end{gather}
By solving \eqsref{eq:local_equilibrium}, one can obtain $\ddfrac{\mb{x}}{\bvarepsilon}$, we are interested in the first row, which can be explicitly written as
\begin{gather}
...
Compared to the previous implementation \citep{Fincato2017,Anjiki2019}, the formulation proposed in this work drastically reduces the size of the local system from \num{20} to \num{2}.
...
The function $U\left(z\right)$ shall satisfy the following conditions \citep[see, e.g.,][\S~2.3 for details]{Hashiguchi2024}.
\begin{gather}
    U\left(z\right)=\left\{
    \begin{array}{ll}
        +\infty, & z=0, \\
...
For brevity, assuming no hardening and no elastic core, and $U\left(z\right)$ takes a general form, then the Jacobian (for the uniaxial model) can be simplified to
\begin{gather}
    \mb{J}=\begin{bmatrix}
        -E & -\sigma^y    \\[2mm]
        -U & 1-\gamma{}U'
...
\end{gather}
We call \eqsref{eq:weak_limit} the `weak numerical continuity condition'
\begin{gather}\label{eq:weak_limit}
    \lim\limits_{z\to0^+}\dfrac{U^2}{U-zU'}=\infty,
\end{gather}
and \eqsref{eq:continuity} the `strong numerical continuity condition'.
